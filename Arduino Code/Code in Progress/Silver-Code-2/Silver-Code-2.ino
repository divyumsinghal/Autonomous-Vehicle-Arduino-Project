// Some necessary stuff
  ...
  // Defining encoders outside the class for speed & Syntax:
...
// Private Member Attributes
  ...
  const int FollowingDistanceMediator = 20;        // Distance arounf which you want the car to be
  ...
  const float radiusOfWheel = 3.4;                 // radius of wheel in cm
...
// Functions needed
  ...
  void checkPositionRelativeToObject() 
  {
    ...
    if (nearestObstacleDistance > FollowingDistanceMediator) 
    {
      carSpeed = min(MaxSpeed, (carSpeed * (100 + nearestObstacleDistance/FollowingDistanceMediator)/100));
    }
    else if (nearestObstacleDistance < FollowingDistanceMediator)
    {
      carSpeed = min(60, (carSpeed * (100 - nearestObstacleDistance/FollowingDistanceMediator)/100));
    }; 
...
// Public member functions: Setup & Loop
 ...
 // Serial.print('.');
    ...
    if (loopCounter == FrequencyOfObstacleTracking/2)
    {
      ...
      message = "A" + String(distanceTravelledByTheCar);
    ...
    // Serial.print("Got into the 20th loop");
      ...
      message = "B" + String(nearestObstacleDistance);
